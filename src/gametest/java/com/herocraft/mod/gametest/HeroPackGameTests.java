...
public class HeroPackGameTests implements FabricGameTest {
	...
	public void serumMarksPendingPowerButDoesNotGrant(GameTestHelper helper) {
		...
		int amp = com.herocraft.mod.hero.mutation.ModSerums.amplifierFor(durability);
		player.addEffect(new net.minecraft.world.effect.MobEffectInstance(
				com.herocraft.mod.hero.mutation.ModMobEffects.UNSTABLE_MUTATION, 1200, amp, false, true, true));

		com.herocraft.mod.hero.mutation.MutationManager.serverTick(player);
		...
		helper.assertTrue(ExperimentalPowers.researchStage(player, durability).atLeast(
				com.herocraft.mod.hero.data.ResearchStage.SERUM_STABILIZED), "research should reach SERUM_STABILIZED");
		helper.succeed();
	}
	...
	public void exposureEventGrantsPermanentPower(GameTestHelper helper) {
		...
		com.herocraft.mod.hero.mutation.MutationManager.triggerExposure(player,
				com.herocraft.mod.hero.MutationTrigger.Kind.EXPLOSION);

		helper.assertTrue(ExperimentalPowers.owns(player, durability), "surviving the exposure event should unlock the power");
		helper.assertTrue(player.getEffect(com.herocraft.mod.hero.mutation.ModMobEffects.UNSTABLE_MUTATION) == null,
				"the unstable effect should be consumed on success");
		helper.assertTrue(ExperimentalPowers.researchStage(player, durability)
				== com.herocraft.mod.hero.data.ResearchStage.MUTATION_CONFIRMED, "research should be MUTATION_CONFIRMED");
		helper.succeed();
	}
	...
	public void wrongExposureKindDoesNotGrant(GameTestHelper helper) {
		...
		com.herocraft.mod.hero.mutation.MutationManager.triggerExposure(player,
				com.herocraft.mod.hero.MutationTrigger.Kind.POWDER_SNOW);
	...
	public void researchNoteAdvancesResearch(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		Power flight = power("power_03_flight");
		com.herocraft.mod.hero.mutation.MutationManager.studyResearchNote(player, flight);
		helper.assertTrue(ExperimentalPowers.researchStage(player, flight)
				.atLeast(com.herocraft.mod.hero.data.ResearchStage.RESEARCH_FOUND), "studying a note should record research");
		helper.succeed();
	}
	...
	public void capacityBlocksFurtherMutation(GameTestHelper helper) {
		...
		int amp = com.herocraft.mod.hero.mutation.ModSerums.amplifierFor(extra);
		player.addEffect(new net.minecraft.world.effect.MobEffectInstance(
				com.herocraft.mod.hero.mutation.ModMobEffects.UNSTABLE_MUTATION, 1200, amp, false, true, true));
		com.herocraft.mod.hero.mutation.MutationManager.serverTick(player);
		com.herocraft.mod.hero.mutation.MutationManager.triggerExposure(player,
				com.herocraft.mod.hero.MutationTrigger.Kind.SUBMERSION);
	...
	public void guideChaptersBuildFromRegistry(GameTestHelper helper) {
		var chapters = com.herocraft.mod.hero.guide.HeroPackGuide.chapters();
		// 13 framing chapters (overview, mutation, structures, devices, combos, Thor, Iron Man,
		// Spider-Man, Max Steel, Punisher, Symbiote, Zombie Raid, Supervillain Raid) + one per power
		helper.assertTrue(chapters.size() == 13 + Powers.count(),
		...
		var index = com.herocraft.mod.hero.guide.HeroPackGuide.index();
	...
	public void researchSiteAndDevicesAreRegistered(GameTestHelper helper) {
		helper.assertTrue(net.minecraft.core.registries.BuiltInRegistries.STRUCTURE_TYPE.containsKey(
				com.herocraft.mod.HeroCraftMod.id("research_site")), "research_site structure type must be registered");
		for (String d : new String[]{"overloaded_redstone_coil", "geological_resonance_chamber", "electromagnetic_coil"}) {
			helper.assertTrue(net.minecraft.core.registries.BuiltInRegistries.BLOCK.containsKey(
					com.herocraft.mod.HeroCraftMod.id(d)), d + " device block must be registered");
		}
		helper.assertTrue(com.herocraft.mod.hero.item.HeroPackItems.GUIDE != null, "HeroPack Guide item must exist");
		helper.succeed();
	}
	...
	public void everyAbilityHasAHandler(GameTestHelper helper) {
		...
		for (Power p : Powers.all()) {
			for (Ability a : p.abilities()) {
				if (!com.herocraft.mod.hero.AbilityHandlers.has(p, a)) {
					missing++;
	...
	public void flightToggleEngagesIndependentHeroFlight(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.hero.power.HeroFlight.isFlying(player), "Flight toggle should engage hero flight");
		helper.assertTrue(player.getAbilities().mayfly, "hero flight should grant mayfly");
		helper.assertFalse(com.herocraft.mod.power.ThorPowers.isFlying(player), "hero flight must be independent of Thor flight");

		AbilityRouter.handleInput(player, 3, true);
		helper.assertFalse(com.herocraft.mod.hero.power.HeroFlight.isFlying(player), "toggling again should disable hero flight");
		helper.succeed();
	}
	...
	@GameTest(template = EMPTY_STRUCTURE)
	public void healingFactorRapidHealRestoresHealth(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		Power heal = power("power_12_healing_factor");
	...
	public void waterAquaticFormGivesWaterBreathing(GameTestHelper helper) {
		...
		com.herocraft.mod.hero.ExperimentalPowers.serverTick(player); // run one toggle tick
	...
	public void switchingPowerEndsHeroFlight(GameTestHelper helper) {
		...
		ExperimentalPowers.setActive(player, flight);
		AbilityRouter.handleInput(player, 3, true);
		helper.assertTrue(com.herocraft.mod.hero.power.HeroFlight.isFlying(player), "should be flying");

		ExperimentalPowers.setActive(player, strength);
		com.herocraft.mod.hero.power.HeroFlight.tick(player);
		helper.assertFalse(com.herocraft.mod.hero.power.HeroFlight.isFlying(player),
	...
	public void ironManSuitsAndRecipesRegistered(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuits.all().size() == 7,
				"expected 7 Iron Man suits");
		for (String id : new String[]{"mark_1", "mark_2", "mark_iii", "mark_4", "mark_v", "mark_6", "mark_vii"}) {
			helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuits.byId(id) != null, id + " missing");
		}
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_42") == null
				&& com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_50") == null,
				"Mark XLII / Mark L must be removed");
		helper.assertTrue(com.herocraft.mod.ironman.fabricator.FabricatorRecipes.all().size() >= 24,
		...
		helper.assertTrue(net.minecraft.core.registries.BuiltInRegistries.MENU.containsKey(
				com.herocraft.mod.HeroCraftMod.id("stark_fabricator")), "fabricator menu type must be registered");
		helper.assertTrue(net.minecraft.core.registries.BuiltInRegistries.BLOCK.containsKey(
				com.herocraft.mod.HeroCraftMod.id("stark_fabricator")), "stark_fabricator block must be registered");
		helper.succeed();
	}
	...
	public void arcReactorGrantsTonyStarkOnceAndIsConsumed(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.hasPower(player), "new player has no Tony Stark power");

		ItemStack reactor = new ItemStack(com.herocraft.mod.ironman.item.IronManItems.ARC_REACTOR, 2);
		player.setItemInHand(InteractionHand.MAIN_HAND, reactor);
		com.herocraft.mod.ironman.item.IronManItems.ARC_REACTOR.use(player.level(), player, InteractionHand.MAIN_HAND);

		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.hasPower(player), "Arc Reactor must grant Tony Stark");
	...
	public void ironManArmorRejectsNonTonyStark(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		var chest = com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
		...
		com.herocraft.mod.ironman.IronManArmor.enforce(player);
		...
		helper.assertFalse(com.herocraft.mod.ironman.ability.IronManAbilityManager.hasContext(player),
	...
	public void tonyStarkPlayerCanEquipAndOperateSuit(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.TonyStark.unlockTech(player, 5);
		var chest = com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
				net.minecraft.world.item.ArmorItem.Type.CHESTPLATE);
		player.setItemSlot(net.minecraft.world.entity.EquipmentSlot.CHEST, new ItemStack(chest));
		...
		helper.assertTrue(com.herocraft.mod.ironman.ability.IronManAbilityManager.hasContext(player),
				"Tony Stark + suit => Iron Man ability context");

		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_iii", 5000f);
		float before = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, AbilitySlot.SLOT_1, true);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, AbilitySlot.SLOT_1, false); // tap R -> Repulsor Blast fires on release
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii") < before,
	...
	public void chargedRepulsorCostsMoreThanTapRepulsor(GameTestHelper helper) {
		...
		player.setItemSlot(net.minecraft.world.entity.EquipmentSlot.CHEST,
				new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
						net.minecraft.world.item.ArmorItem.Type.CHESTPLATE)));

		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_iii", 10_000f);
		float start = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");

		// tap R -> ordinary Repulsor Blast
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, AbilitySlot.SLOT_1, true);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, AbilitySlot.SLOT_1, false);
		float afterTap = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");

		// hold R for >2s then release -> Charged Repulsor (simulate the hold by backdating the start tick)
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, AbilitySlot.SLOT_1, true);
		com.herocraft.mod.ironman.TonyStark.state(player).chargeStartTick = player.level().getGameTime() - 60L;
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, AbilitySlot.SLOT_1, false);
		float afterCharge = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");
	...
	public void suitPlatformDeployRechargesAndRepairs(GameTestHelper helper) {
		...
		helper.setBlock(pos, com.herocraft.mod.ironman.IronManBlocks.IRON_MAN_SUIT_PLATFORM);
		var be = (com.herocraft.mod.ironman.fabricator.IronManSuitPlatformBlockEntity)
		...
		for (net.minecraft.world.item.ArmorItem.Type t : new net.minecraft.world.item.ArmorItem.Type[] {
			...
			be.store(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii", t)));
		}
		be.addEnergy(5000);
		com.herocraft.mod.ironman.IronManEnergy.setIntegrity(player, "mark_iii", 10f);

		helper.assertTrue(be.deployTo(player), "platform must deploy the stored suit");
		helper.assertTrue(com.herocraft.mod.ironman.IronManArmor.wearingFullSuit(player, "mark_iii"),
				"player must be wearing the full Mark III after deploy");
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii") >= 5000f,
				"deploy must transfer the platform's stored charge");
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.integrity(player, "mark_iii") >= 99f,
	...
	public void starkFabricatorConsumesReactorCoreForEnergy(GameTestHelper helper) {
		net.minecraft.core.BlockPos pos = new net.minecraft.core.BlockPos(2, 2, 2);
		helper.setBlock(pos, com.herocraft.mod.ironman.IronManBlocks.STARK_FABRICATOR);
		var be = (com.herocraft.mod.ironman.fabricator.StarkFabricatorBlockEntity) helper.getBlockEntity(pos);
		int before = be.energy();
		be.setItem(0, new ItemStack(com.herocraft.mod.ironman.item.IronManItems.REACTOR_CORE, 2));
		com.herocraft.mod.ironman.fabricator.StarkFabricatorBlockEntity.serverTick(
	...
	public void chlorokinesisDetectsNatureGround(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.hero.power.p22.PlantManipulationHandlers.onNatureGround(
		...
		helper.assertFalse(com.herocraft.mod.hero.power.p22.PlantManipulationHandlers.onNatureGround(
	...
	public void ironManCallArmorContextWithoutWearingASuit(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.TonyStark.markBuilt(player, com.herocraft.mod.ironman.suit.IronManSuits.MARK_III);
		helper.assertTrue(com.herocraft.mod.ironman.ability.IronManAbilityManager.hasContext(player),
	...
	public void arcReactorRemovalReturnsPowerAndItem(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		com.herocraft.mod.ironman.TonyStark.grant(player);
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.hasPower(player), "granted");

		// removal is blocked while a suit is on
		player.setItemSlot(net.minecraft.world.entity.EquipmentSlot.CHEST,
				new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
						net.minecraft.world.item.ArmorItem.Type.CHESTPLATE)));
		helper.assertTrue(com.herocraft.mod.ironman.IronManArmor.wearingAnyIronMan(player),
		...
		com.herocraft.mod.ironman.TonyStark.revoke(player);
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.hasPower(player),
				"revoke must clear the Tony Stark power");
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.techLevel(player) == 0
				&& com.herocraft.mod.ironman.TonyStark.state(player).builtSuits.isEmpty(),
				"revoke must reset tech + built suits");

		ItemStack back = new ItemStack(com.herocraft.mod.ironman.item.IronManItems.ARC_REACTOR);
	...
	public void suitCallLaunchesCouriersFromInventory(GameTestHelper helper) {
		...
		for (net.minecraft.world.item.ArmorItem.Type t : net.minecraft.world.item.ArmorItem.Type.values()) {
			if (com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii", t) != null) {
				player.getInventory().add(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii", t)));
			}
		}
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuitSummonManager.summonBest(player),
				"summonBest should succeed with the pieces in the inventory");
		long couriers = helper.getLevel().getEntitiesOfClass(
				com.herocraft.mod.ironman.entity.IronManSuitPartEntity.class,
		...
		helper.assertTrue(player.getInventory().countItem(
				com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii", net.minecraft.world.item.ArmorItem.Type.HELMET)) == 0,
	...
	public void suitCallLaunchesCouriersFromNearbyPlatform(GameTestHelper helper) {
		...
		helper.setBlock(platPos, com.herocraft.mod.ironman.IronManBlocks.IRON_MAN_SUIT_PLATFORM);
		var be = (com.herocraft.mod.ironman.fabricator.IronManSuitPlatformBlockEntity) helper.getBlockEntity(platPos);
		for (net.minecraft.world.item.ArmorItem.Type t : net.minecraft.world.item.ArmorItem.Type.values()) {
			if (com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii", t) != null) {
				be.store(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii", t)));
			}
	...
	public void repulsorBarrierBlocksMostDamage(GameTestHelper helper) {
		...
		for (net.minecraft.world.item.ArmorItem.Type t : net.minecraft.world.item.ArmorItem.Type.values()) {
			if (com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii", t) != null) {
				player.setItemSlot(com.herocraft.mod.ironman.suit.IronManSuitUpManager.slotFor(t),
						new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii", t)));
			}
		}
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_iii", 10_000f);
		com.herocraft.mod.ironman.IronManEnergy.setIntegrity(player, "mark_iii", 100f);

		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, AbilitySlot.SLOT_2, true); // hold barrier down
		helper.assertTrue(com.herocraft.mod.ironman.ability.IronManAbilities.barrierActive(player),
				"holding slot 2 must raise the Repulsor Barrier");

		// it drains energy every tick it is held
		float e0 = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");
		com.herocraft.mod.ironman.ability.IronManAbilities.tickBarrier(player, com.herocraft.mod.ironman.suit.IronManSuits.MARK_III);
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii") < e0,
		...
		com.herocraft.mod.ironman.IronManSuitTicker.tick(player);
		helper.assertFalse(com.herocraft.mod.ironman.ability.IronManAbilities.barrierActive(player),
				"taking the suit off must cut the barrier");

		// releasing the key when it is up starts the 6 s cooldown
		com.herocraft.mod.ironman.TonyStark.state(player).barrierHeld = true;
		com.herocraft.mod.ironman.ability.IronManAbilities.stopBarrier(player, com.herocraft.mod.ironman.suit.IronManSuits.MARK_III, true);
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.abilityReady(player, "mark_iii",
				com.herocraft.mod.ironman.ability.IronManAbilities.REPULSOR_BARRIER),
	...
	public void suitChargeTravelsWithTheArmour(GameTestHelper helper) {
		...
		ItemStack chest = new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
				net.minecraft.world.item.ArmorItem.Type.CHESTPLATE));
		com.herocraft.mod.ironman.IronManEnergy.stampStack(chest, 3_000f, 55f);
		...
		com.herocraft.mod.ironman.suit.IronManSuitUpManager.beginSuitUp(player, "mark_iii");
		float pool = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");
		helper.assertTrue(Math.abs(pool - 3_000f) < 1f,
				"suit-up must adopt the chestplate's carried charge (3000), got " + pool);
		helper.assertTrue(Math.abs(com.herocraft.mod.ironman.IronManEnergy.integrity(player, "mark_iii") - 55f) < 1f,
	...
	public void callSuitFromInventoryEquipsImmediately(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuitCall.fullyInInventory(player, "mark_iii"),
				"the four pieces are in the pack");
		com.herocraft.mod.ironman.suit.IronManSuitCall.execute(player, "mark_iii",
				com.herocraft.mod.network.IronManSuitListPayload.SOURCE_INVENTORY);
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuitUpManager.inTransition(player),
	...
	public void callBestPullsASuitOffABoundPlatform(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuitCall.callBest(player),
	...
	public void suitAutoRecoversToPlatformOnDeath(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_iii", 4200f);
		com.herocraft.mod.ironman.IronManEnergy.setIntegrity(player, "mark_iii",
				com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_iii"));
		...
		com.herocraft.mod.ironman.suit.IronManSuitCall.recoverSuitOnDeath(player);
		...
		float half = com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_iii") * 0.5f;
	...
	public void ironManSuitGrantsStrengthProRata(GameTestHelper helper) {
		...
		player.setItemSlot(net.minecraft.world.entity.EquipmentSlot.CHEST,
				new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
						net.minecraft.world.item.ArmorItem.Type.CHESTPLATE)));
		com.herocraft.mod.ironman.IronManPassives.tick(player);
		double oneOfFour = atk.getModifier(com.herocraft.mod.HeroCraftMod.id("iron_man_strength")).amount();
		...
		player.setItemSlot(net.minecraft.world.entity.EquipmentSlot.FEET,
				new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
						net.minecraft.world.item.ArmorItem.Type.BOOTS)));
		com.herocraft.mod.ironman.IronManPassives.tick(player);
		double full = atk.getModifier(com.herocraft.mod.HeroCraftMod.id("iron_man_strength")).amount();
		...
		com.herocraft.mod.ironman.IronManPassives.tick(player);
		helper.assertTrue(atk.getModifier(com.herocraft.mod.HeroCraftMod.id("iron_man_strength")) == null,
	...
	private static void giveWholeSuit(ServerPlayer player, String suitId) {
		for (net.minecraft.world.item.ArmorItem.Type t : new net.minecraft.world.item.ArmorItem.Type[] {
				net.minecraft.world.item.ArmorItem.Type.HELMET, net.minecraft.world.item.ArmorItem.Type.CHESTPLATE,
				net.minecraft.world.item.ArmorItem.Type.LEGGINGS, net.minecraft.world.item.ArmorItem.Type.BOOTS }) {
			player.getInventory().add(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor(suitId, t)));
		}
	}
	...
	private static int countIronManPieces(ServerPlayer player, String suitId) {
		...
		for (ItemStack stack : player.getInventory().items) {
			if (stack.getItem() instanceof com.herocraft.mod.ironman.item.IronManArmorItem p
	...
	public void interruptedSuitUpKeepsThePiecesInTheInventory(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuitUpManager.beginSuitUp(player, "mark_iii"),
		...
		var s = com.herocraft.mod.ironman.TonyStark.state(player);
		s.transitionSuit = "";
		s.transitionTicks = 0;
		s.transitionMask = 0;
	...
	public void suitUpEvictsOrdinaryArmourInsteadOfDeletingIt(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuitUpManager.beginSuitUp(player, "mark_iii"),
				"suit-up should start");
		for (int i = 0; i < 200 && com.herocraft.mod.ironman.suit.IronManSuitUpManager.inTransition(player); i++) {
			com.herocraft.mod.ironman.suit.IronManSuitUpManager.tick(player);
		}
	...
	public void deathRecoveryOnlyTakesTheIronManPieces(GameTestHelper helper) {
		...
		var registry = com.herocraft.mod.ironman.data.StarkPlatformRegistry.get(level);
	...
	public void breakingASuitPlatformDropsTheSuitItHeld(GameTestHelper helper) {
		...
		helper.setBlock(platPos, com.herocraft.mod.ironman.IronManBlocks.IRON_MAN_SUIT_PLATFORM);
		var be = (com.herocraft.mod.ironman.fabricator.IronManSuitPlatformBlockEntity) helper.getBlockEntity(platPos);
		var chestItem = com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
				net.minecraft.world.item.ArmorItem.Type.CHESTPLATE);
	...
	public void aSuitPlatformRefusesToMixMarks(GameTestHelper helper) {
		...
		helper.assertTrue(be.store(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
				net.minecraft.world.item.ArmorItem.Type.CHESTPLATE))), "first piece should be accepted");
		helper.assertFalse(be.store(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_v",
	...
	private static void giveFullSuit(ServerPlayer player, String suitId) {
		for (net.minecraft.world.item.ArmorItem.Type t : net.minecraft.world.item.ArmorItem.Type.values()) {
			var item = com.herocraft.mod.ironman.item.IronManItems.armor(suitId, t);
			if (item != null) {
				player.setItemSlot(com.herocraft.mod.ironman.suit.IronManSuitUpManager.slotFor(t), new ItemStack(item));
			}
		}
	}
	...
	public void storingSuitRefusesWithoutFourFreeMainSlots(GameTestHelper helper) {
		...
		helper.assertFalse(com.herocraft.mod.ironman.suit.IronManSuitUpManager.beginSuitDown(player, "mark_iii"),
	...
	public void storingSuitLandsInMainInventoryNotHotbar(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuitUpManager.beginSuitDown(player, "mark_iii"),
				"suit-down should start with room available");
		for (int i = 0; i < 200 && com.herocraft.mod.ironman.suit.IronManSuitUpManager.inTransition(player); i++) {
			com.herocraft.mod.ironman.suit.IronManSuitUpManager.tick(player);
		}
		helper.assertTrue(!com.herocraft.mod.ironman.IronManArmor.wearingAnyIronMan(player), "the suit should be off");
		var hotbar = player.getInventory().items;
		boolean anyInHotbar = false;
		for (int i = 0; i < 9; i++) {
			if (hotbar.get(i).getItem() instanceof com.herocraft.mod.ironman.item.IronManArmorItem) {
				anyInHotbar = true;
			}
		}
	...
	public void integrityFailureAppliesSlownessAndWeakness(GameTestHelper helper) {
		...
		giveFullSuit(player, "mark_iii");
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_iii", 5000f);
		com.herocraft.mod.ironman.IronManEnergy.setIntegrity(player, "mark_iii", 0f);
	...
	public void carriedOnlySuitIsRecoveredOnDeathToo(GameTestHelper helper) {
		...
		for (net.minecraft.world.item.ArmorItem.Type t : new net.minecraft.world.item.ArmorItem.Type[] { net.minecraft.world.item.ArmorItem.Type.HELMET, net.minecraft.world.item.ArmorItem.Type.CHESTPLATE, net.minecraft.world.item.ArmorItem.Type.LEGGINGS, net.minecraft.world.item.ArmorItem.Type.BOOTS }) {
			ItemStack stack = new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii", t));
			com.herocraft.mod.ironman.IronManEnergy.stampStack(stack, 5000f,
					com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_iii"));
		...
		float half3 = com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_iii") * 0.5f;
		...
		for (ItemStack s : player.getInventory().items) {
			if (s.getItem() instanceof com.herocraft.mod.ironman.item.IronManArmorItem) {
				leftInPack++;
			}
		}
	...
	public void mark1And2AreRegisteredAndCraftableAtTechZero(GameTestHelper helper) {
		var mark1 = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_1");
		var mark2 = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_2");
		...
		for (net.minecraft.world.item.ArmorItem.Type t : new net.minecraft.world.item.ArmorItem.Type[] { net.minecraft.world.item.ArmorItem.Type.HELMET, net.minecraft.world.item.ArmorItem.Type.CHESTPLATE, net.minecraft.world.item.ArmorItem.Type.LEGGINGS, net.minecraft.world.item.ArmorItem.Type.BOOTS }) {
			helper.assertTrue(com.herocraft.mod.ironman.item.IronManItems.armor("mark_1", t) != null, "mark_1 " + t + " item missing");
			helper.assertTrue(com.herocraft.mod.ironman.item.IronManItems.armor("mark_2", t) != null, "mark_2 " + t + " item missing");
		}
		helper.succeed();
	}
	...
	public void mark1And2GrantFiveUnarmedDamageFullySuited(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.IronManPassives.tick(player);
		double total = 1.0 /* vanilla base fist */ + atk.getModifier(com.herocraft.mod.HeroCraftMod.id("iron_man_strength")).amount();
	...
	public void mark2WindupRepulsorDoesNotFireInstantly(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_2", 5000f);
		float before = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_2");

		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, true);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, false);
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_2") == before,
		...
		var suit = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_2");
		com.herocraft.mod.ironman.TonyStark.state(player).repulsorWindupAt = player.level().getGameTime();
		com.herocraft.mod.ironman.ability.IronManAbilities.tickRepulsorWindup(player, suit);
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_2") < before,
	...
	public void mark2SystemsFreezeAboveAltitudeCeiling(GameTestHelper helper) {
		...
		giveFullSuit(player, "mark_2");
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_2", 5000f);
		com.herocraft.mod.ironman.IronManEnergy.setIntegrity(player, "mark_2", 500f);
		player.setAttached(com.herocraft.mod.attachment.ModAttachments.IRON_MAN_FLYING, true);
		player.setPos(player.getX(), 160.0, player.getZ());

		com.herocraft.mod.ironman.IronManSuitTicker.tick(player);

		helper.assertFalse(com.herocraft.mod.ironman.IronManFlight.isFlying(player),
	...
	public void rocketAbilityDealsFifteenDamage(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.TonyStark.grant(player);
		giveFullSuit(player, "mark_1");
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_1", 5000f);

		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_4, true);

		long rockets = helper.getLevel().getEntitiesOfClass(com.herocraft.mod.ironman.entity.IronManMissileEntity.class,
	...
	public void mobHighlightTogglePersistsUntilPressedAgain(GameTestHelper helper) {
		...
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.state(player).mobHighlightOn, "starts off");
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_5, true);
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.state(player).mobHighlightOn, "V should toggle it on");
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_5, true);
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.state(player).mobHighlightOn, "a second V toggles it back off");
		helper.succeed();
	}
	...
	public void mark1CannotStartRepulsorFlightFromDoubleJump(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.TonyStark.grant(player);
		giveFullSuit(player, "mark_1");
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_1", 3000f);

		com.herocraft.mod.ironman.IronManFlight.toggle(player); // the double-tap-jump entry point
		helper.assertFalse(com.herocraft.mod.ironman.IronManFlight.isFlying(player),
				"Mark 1 must not take off from double-tap jump -- only via its flight ability");

		// its own X flight ability still works
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_3, true);
		helper.assertTrue(com.herocraft.mod.ironman.IronManFlight.isFlying(player),
	...
	public void repulsorEnergyCostsAreFlatFiftyAndHundred(GameTestHelper helper) {
		...
		float a = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, true);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, false);
		float b = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");
		helper.assertTrue(Math.abs((a - b) - com.herocraft.mod.ironman.ability.IronManAbilities.REPULSOR_ENERGY) < 0.01f,
				"a tap Repulsor must cost exactly REPULSOR_ENERGY (80, 'changes 18'), spent " + (a - b));

		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, true);
		com.herocraft.mod.ironman.TonyStark.state(player).chargeStartTick = player.level().getGameTime() - 60L;
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, false);
		float c = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");
		helper.assertTrue(Math.abs((b - c) - com.herocraft.mod.ironman.ability.IronManAbilities.CHARGED_REPULSOR_ENERGY) < 0.01f,
	...
	public void markThreeIntegrityPoolIsFifteenHundred(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_1") == 300f,
				"Mark 1 max integrity must be 300");
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_2") == 420f,
				"Mark 2 max integrity must be 420");
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_iii") == 600f,
				"Mark III max integrity must be 600");
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_4") == 700f,
				"Mark 4 max integrity must be 700");
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_v") == 550f,
				"Mark V max integrity must be 550");
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_6") == 800f,
				"Mark 6 max integrity must be 800");
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("mark_vii") == 950f,
				"Mark VII max integrity must be 950");
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.maxIntegrity("unknown_suit") == 500f,
				"an unknown suit id falls back to the 500 default");

		ServerPlayer player = survivalMockPlayer(helper);
		com.herocraft.mod.ironman.TonyStark.grant(player);
		com.herocraft.mod.ironman.IronManEnergy.setIntegrity(player, "mark_iii", 9000f); // over-set -> must clamp to 600
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.integrity(player, "mark_iii") == 600f,
				"integrity must clamp to the per-suit max, got "
						+ com.herocraft.mod.ironman.IronManEnergy.integrity(player, "mark_iii"));
		helper.succeed();
	}
	...
	public void advancedMarksCarryMobHighlightInsteadOfTargeting(GameTestHelper helper) {
		for (String id : new String[]{"mark_iii", "mark_v", "mark_4", "mark_6"}) {
			var suit = com.herocraft.mod.ironman.suit.IronManSuits.byId(id);
			helper.assertTrue(com.herocraft.mod.ironman.ability.IronManAbilities.MOB_HIGHLIGHT_TOGGLE.equals(suit.abilityInSlot(5)),
					id + " slot 5 must be the mob-highlight toggle");
			for (int slot = 1; slot <= 6; slot++) {
				helper.assertFalse(com.herocraft.mod.ironman.ability.IronManAbilities.TARGETING_MODE.equals(suit.abilityInSlot(slot)),
	...
	public void mobHighlightClearsWhenSuitLosesPower(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_iii", 5000f);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_5, true);
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.state(player).mobHighlightOn, "toggled on");

		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_iii", 0f); // depleted
		com.herocraft.mod.ironman.IronManSuitTicker.tick(player);
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.state(player).mobHighlightOn,
	...
	public void markFourIsRegisteredWithWristLaser(GameTestHelper helper) {
		var m4 = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_4");
		...
		var m3 = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_iii");
		for (int s = 1; s <= 6; s++) {
			helper.assertTrue(java.util.Objects.equals(m4.abilityInSlot(s), m3.abilityInSlot(s)),
					"mark_4 slot " + s + " must match Mark III");
		}
		helper.assertTrue(com.herocraft.mod.ironman.item.IronManItems.armor("mark_4",
				net.minecraft.world.item.ArmorItem.Type.CHESTPLATE) != null, "mark_4 chestplate item missing");
		helper.assertTrue(com.herocraft.mod.ironman.item.IronManItems.MARK_4_BLUEPRINT != null, "mark_4 blueprint missing");
		helper.succeed();
	}
	...
	public void markVRedefinedAsMovieMarkFive(GameTestHelper helper) {
		var mv = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_v");
		var m2 = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_2");
		...
		for (int s = 1; s <= 6; s++) {
			if (s == 3) {
				...
				helper.assertTrue(com.herocraft.mod.ironman.ability.IronManAbilities.BLADE.equals(mv.abilityInSlot(3)),
		...
		helper.assertTrue(mv.suitUpType() == com.herocraft.mod.ironman.suit.SuitUpType.SUITCASE_MOVIE,
	...
	public void markOneFlightBurstGoesOnCooldown(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_1", 3000f);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_3, true);
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.state(player).timedFlightUntil > 0L, "burst must start");
		com.herocraft.mod.ironman.ability.IronManAbilities.endTimedFlight(player, "mark_1");
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.state(player).timedFlightUntil == 0L, "burst must clear");
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.abilityReady(player, "mark_1",
				com.herocraft.mod.ironman.ability.IronManAbilities.TIMED_FLIGHT), "flight burst must be on cooldown after it ends");
		helper.succeed();
	}
	...
	public void abilityWithoutEnergyIsRefusedCleanly(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.TonyStark.grant(player);
		giveFullSuit(player, "mark_iii");
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_iii", 10f); // less than a repulsor's 50
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, true);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, false);
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii") == 10f,
	...
	public void wornReactorTrickleIsTenthOfAPercent(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.TonyStark.grant(player);
		giveFullSuit(player, "mark_iii");
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_iii", 100f);
		var suit = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_iii");
		com.herocraft.mod.ironman.IronManEnergy.tickRecharge(player, suit);
		float gained = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii") - 100f;
		...
		com.herocraft.mod.ironman.IronManEnergy.setIntegrity(player, "mark_iii", 100f);
		com.herocraft.mod.ironman.IronManEnergy.tickArmorRegen(player, suit);
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.integrity(player, "mark_iii") > 100f,
	...
	public void markSixAndSevenAreConfigured(GameTestHelper helper) {
		var m6 = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_6");
		...
		helper.assertTrue(com.herocraft.mod.ironman.item.IronManItems.armor("mark_6",
				net.minecraft.world.item.ArmorItem.Type.CHESTPLATE) != null, "mark_6 chest item");

		var m7 = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_vii");
		...
		helper.assertTrue(com.herocraft.mod.ironman.ability.IronManAbilities.WEAPON_WHEEL_SLOT.equals(m7.abilityInSlot(3)),
	...
	public void cheapSuitSpendsLessEnergy(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_6", 5000f);
		float before = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_6");
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, true);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, false);
		float spent = before - com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_6");
		float expected = com.herocraft.mod.ironman.ability.IronManAbilities.REPULSOR_ENERGY
				* com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_6").energyCostMultiplier();
	...
	public void overloadedSuitCannotFireRepulsor(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_4", 5000f);
		com.herocraft.mod.ironman.TonyStark.setOverloadUntil(player, player.level().getGameTime() + 200L);
		float before = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_4");
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, true);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_1, false);
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_4") == before,
	...
	public void markOneIsAHeavyPrototype(GameTestHelper helper) {
		var m1 = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_1");
		...
		for (String id : new String[]{"mark_2", "mark_iii", "mark_4", "mark_v", "mark_6", "mark_vii"}) {
			helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuits.byId(id).helmetNightVision(),
	...
	public void airTanksAreConfigured(GameTestHelper helper) {
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_2").airTankSeconds() == 120, "mark_2 air");
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_iii").airTankSeconds() == 180, "mark_iii air");
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_4").airTankSeconds() == 180, "mark_4 air");
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_6").airTankSeconds() == 300, "mark_6 air");
		helper.assertTrue(com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_vii").airTankSeconds() == 420, "mark_vii air");
		helper.succeed();
	}
	...
	public void supersonicFlightRework(GameTestHelper helper) {
		helper.assertTrue(com.herocraft.mod.ironman.ability.IronManAbilities.SUPERSONIC_TICKS == 400,
				"supersonic burst must be 20 s");
		helper.assertTrue(com.herocraft.mod.ironman.ability.IronManAbilities.SUPERSONIC_COOLDOWN_TICKS == 200,
	...
	public void microMissilesFireOneAtATime(GameTestHelper helper) {
		...
		float before = com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii");
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_3, true);
		int queued = com.herocraft.mod.ironman.TonyStark.state(player).pendingMissiles;
		helper.assertTrue(queued == suit.missileCount(),
				"pressing Micro-Missiles must queue missileCount() missiles at once, got " + queued);
		helper.assertTrue(before - com.herocraft.mod.ironman.IronManEnergy.energy(player, "mark_iii")
		...
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_3, true);
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.state(player).pendingMissiles == queued,
				"a second press must not re-arm the volley");
		// the ticker's shutdown must clear a pending volley
		com.herocraft.mod.ironman.IronManSuitTicker.shutDownAllSystems(player,
				com.herocraft.mod.ironman.TonyStark.state(player), suit);
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.state(player).pendingMissiles == 0,
	...
	public void changes18SuitTuning(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.ironman.ability.IronManAbilities.CHARGED_REPULSOR_DAMAGE_MULTIPLIER == 3.0f,
	...
	public void changes22PlatformRegen(GameTestHelper helper) {
		...
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.PLATFORM_FRACTION_PER_SECOND == 0.001f,
				"a platform charges a flat 0.1% per second");
		for (var suit : new com.herocraft.mod.ironman.suit.IronManSuit[] { m1, m2, m7 }) {
			helper.assertTrue(Math.abs(com.herocraft.mod.ironman.IronManEnergy.platformEnergyPerSecond(suit)
					- suit.energyCapacity() * 0.001f) < 1e-4f,
					"platform energy regen is 0.1% of capacity for " + suit.id());
			helper.assertTrue(Math.abs(com.herocraft.mod.ironman.IronManEnergy.platformIntegrityPerSecond(suit)
		...
		helper.assertTrue(com.herocraft.mod.ironman.IronManEnergy.platformIntegrityPerSecond(m1) > 0f
				&& com.herocraft.mod.ironman.IronManEnergy.platformIntegrityPerSecond(m2) > 0f,
	...
	public void changes22FlamethrowerAndFabricator(GameTestHelper helper) {
		...
		helper.assertTrue(Math.abs(
				com.herocraft.mod.ironman.ability.IronManAbilities.flamethrowerMaxHeat(m1) - 300f) < 1e-3f,
				"Mark 1 flamethrower heat capacity is 300");

		var chest = com.herocraft.mod.ironman.fabricator.FabricatorRecipes.byId("iron_man_mark_iii_chestplate");
		helper.assertTrue(chest != null && chest.energyCost()
				== com.herocraft.mod.ironman.fabricator.FabricatorRecipes.MAX_ENERGY,
		...
		var circuit = com.herocraft.mod.ironman.fabricator.FabricatorRecipes.byId("stark_circuit");
		helper.assertTrue(circuit != null
				&& circuit.energyCost() < com.herocraft.mod.ironman.fabricator.FabricatorRecipes.MAX_ENERGY,
	...
	public void changes22RepulsorBoots(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		ItemStack repulsor = new ItemStack(com.herocraft.mod.ironman.item.IronManItems.REPULSOR);

		helper.assertTrue(repulsor.getMaxStackSize() > 1,
				"the Repulsor must still stack -- the armour recipes need several at once");
		helper.assertTrue(com.herocraft.mod.ironman.item.IronManItems.REPULSOR
				instanceof com.herocraft.mod.ironman.item.RepulsorItem r
				&& r.getEquipmentSlot() == net.minecraft.world.entity.EquipmentSlot.FEET,
				"the Repulsor equips into the boots slot");

		helper.assertTrue(!com.herocraft.mod.ironman.RepulsorBoots.worn(player),
				"no boots on -- not wearing a Repulsor");
		helper.assertTrue(!com.herocraft.mod.ironman.RepulsorBoots.toggle(player),
		...
		helper.assertTrue(com.herocraft.mod.ironman.RepulsorBoots.worn(player), "Repulsor is worn");
		// no Tony Stark power granted anywhere in this test -- that is the point
		helper.assertTrue(!com.herocraft.mod.ironman.TonyStark.hasPower(player),
				"repulsor flight must not need the Tony Stark power");
		helper.assertTrue(com.herocraft.mod.ironman.RepulsorBoots.toggle(player)
				&& com.herocraft.mod.ironman.RepulsorBoots.isFlying(player), "boots flight engages");
		helper.assertTrue(com.herocraft.mod.ironman.RepulsorBoots.toggle(player)
				&& !com.herocraft.mod.ironman.RepulsorBoots.isFlying(player), "and toggles back off");

		// exactly half the Mark 2's flight numbers
		var m2 = com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_2");
		helper.assertTrue(Math.abs(com.herocraft.mod.ironman.RepulsorBoots.FLIGHT_SPEED
				- m2.flightSpeed() / 2f) < 1e-5f, "boots fly at 50% of the Mark 2's speed");
		helper.assertTrue(Math.abs(com.herocraft.mod.ironman.RepulsorBoots.FLIGHT_ACCELERATION
	...
	public void changes19FaceplateAndBlades(GameTestHelper helper) {
		...
		com.herocraft.mod.ironman.IronManEnergy.setEnergy(player, "mark_v", 5000f);

		// faceplate
		com.herocraft.mod.ironman.IronManFaceplate.toggle(player);
		helper.assertTrue(com.herocraft.mod.ironman.IronManFaceplate.isOpen(player), "H opens the faceplate");
		...
		com.herocraft.mod.ironman.IronManFaceplate.reconcile(player);
		helper.assertFalse(com.herocraft.mod.ironman.IronManFaceplate.isOpen(player),
		...
		var atk = player.getAttribute(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_DAMAGE);
		com.herocraft.mod.ironman.ability.IronManAbilityManager.handle(player, com.herocraft.mod.hero.AbilitySlot.SLOT_3, true);
		helper.assertTrue(com.herocraft.mod.ironman.IronManBlade.active(player), "X extends the blades");
		com.herocraft.mod.ironman.IronManBlade.tick(player);
		var mod = atk.getModifier(com.herocraft.mod.HeroCraftMod.id("iron_man_blade_strength"));
		...
		com.herocraft.mod.ironman.IronManSuitTicker.shutDownAllSystems(player,
				com.herocraft.mod.ironman.TonyStark.state(player), com.herocraft.mod.ironman.suit.IronManSuits.byId("mark_v"));
		com.herocraft.mod.ironman.IronManBlade.tick(player);
		helper.assertFalse(com.herocraft.mod.ironman.IronManBlade.active(player), "shutdown retracts the blades");
		helper.assertTrue(atk.getModifier(com.herocraft.mod.HeroCraftMod.id("iron_man_blade_strength")) == null,
	...
	public void changes19CallAndFabricatorRecipes(GameTestHelper helper) {
		for (String id : new String[] { "mark_2", "mark_4", "mark_6", "mark_iii", "mark_v", "mark_vii" }) {
			helper.assertTrue(com.herocraft.mod.ironman.fabricator.FabricatorRecipes.hasArmorRecipes(id),
					id + " must have Fabricator armour recipes");
		}
		helper.assertFalse(com.herocraft.mod.ironman.fabricator.FabricatorRecipes.hasArmorRecipes("mark_1"),
		...
		for (net.minecraft.world.item.ArmorItem.Type t : new net.minecraft.world.item.ArmorItem.Type[] {
				net.minecraft.world.item.ArmorItem.Type.HELMET, net.minecraft.world.item.ArmorItem.Type.CHESTPLATE,
				net.minecraft.world.item.ArmorItem.Type.LEGGINGS, net.minecraft.world.item.ArmorItem.Type.BOOTS }) {
			player.getInventory().add(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_1", t)));
		}
		boolean equipped = com.herocraft.mod.ironman.suit.IronManSuitCall.autoEquipInventorySuit(player);
	...
	public void powerSuppressorStripsEverything(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		com.herocraft.mod.ironman.TonyStark.grant(player);
		com.herocraft.mod.worthiness.Worthiness.setScore(player, 100);
		com.herocraft.mod.hero.ExperimentalPowers.grant(player,
				com.herocraft.mod.hero.Powers.all().iterator().next());
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.hasPower(player)
				&& com.herocraft.mod.worthiness.Worthiness.isWorthy(player)
				&& !com.herocraft.mod.hero.ExperimentalPowers.state(player).ownedPowers.isEmpty(),
		...
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.hasPower(player), "Tony Stark power removed");
		helper.assertFalse(com.herocraft.mod.worthiness.Worthiness.isWorthy(player), "worthiness reset");
		helper.assertTrue(com.herocraft.mod.hero.ExperimentalPowers.state(player).ownedPowers.isEmpty(),
	...
	public void protocolPhoenixCooldownState(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		com.herocraft.mod.ironman.TonyStark.grant(player);
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.phoenixReady(player), "Phoenix starts ready");
		helper.assertFalse(com.herocraft.mod.ironman.ProtocolPhoenix.incapacitated(player), "not incapacitated by default");
		com.herocraft.mod.ironman.TonyStark.setPhoenixReadyAt(player,
				player.level().getGameTime() + com.herocraft.mod.ironman.ProtocolPhoenix.COOLDOWN_TICKS);
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.phoenixReady(player), "Phoenix on cooldown after activation");
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.phoenixCooldownRemaining(player) > 250 * 20,
		...
		var copied = com.herocraft.mod.ironman.TonyStark.state(player).copy();
		helper.assertTrue(copied.phoenixReadyAt == com.herocraft.mod.ironman.TonyStark.state(player).phoenixReadyAt,
	...
	private static net.minecraft.world.inventory.CraftingMenu markOneHelmetGrid(GameTestHelper helper,
		...
		ItemStack circuit = new ItemStack(com.herocraft.mod.ironman.item.IronManItems.BASIC_CIRCUIT);
	...
	public void ironManSuitIsNotCraftableWithoutTonyStark(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		helper.assertFalse(com.herocraft.mod.ironman.TonyStark.hasPower(player), "no power yet");
		...
		helper.assertTrue(menu.getSlot(0).getItem().getItem()
						== com.herocraft.mod.ironman.item.IronManItems.armor("mark_1",
								net.minecraft.world.item.ArmorItem.Type.HELMET),
	...
	public void ironManCraftingGateCoversSuitsOnly(GameTestHelper helper) {
		...
		for (String id : new String[]{"mark_1", "mark_2", "mark_iii", "mark_4", "mark_v", "mark_6", "mark_vii"}) {
			for (net.minecraft.world.item.ArmorItem.Type type : net.minecraft.world.item.ArmorItem.Type.values()) {
				var piece = com.herocraft.mod.ironman.item.IronManItems.armor(id, type);
				if (piece == null) {
					continue;
				}
				helper.assertTrue(com.herocraft.mod.ironman.IronManCrafting.requiresTonyStark(new ItemStack(piece)),
						id + " " + type + " must require the Tony Stark power to craft");
			}
		}
		helper.assertTrue(com.herocraft.mod.ironman.IronManCrafting.requiresTonyStark(
				new ItemStack(com.herocraft.mod.ironman.item.IronManItems.MARK_V_SUITCASE)),
		...
		for (net.minecraft.world.item.Item open : new net.minecraft.world.item.Item[]{
				com.herocraft.mod.ironman.item.IronManItems.ARC_REACTOR,
				com.herocraft.mod.ironman.item.IronManItems.BASIC_CIRCUIT,
				com.herocraft.mod.ironman.item.IronManItems.BLANK_BLUEPRINT}) {
			helper.assertFalse(com.herocraft.mod.ironman.IronManCrafting.requiresTonyStark(new ItemStack(open)),
	...
	public void blankBlueprintProgressionIsLinear(GameTestHelper helper) {
		...
		helper.assertTrue(player.getInventory().countItem(com.herocraft.mod.ironman.item.IronManItems.BLANK_BLUEPRINT) == 1,
				"gaining the power gives one Blank Blueprint");

		// Mark 1 has no prerequisite -- stamping it works immediately.
		player.getInventory().add(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.BLANK_BLUEPRINT));
		com.herocraft.mod.ironman.TonyStark.stampBlueprint(player, "mark_1");
		helper.assertTrue(player.getInventory().countItem(com.herocraft.mod.ironman.item.IronManItems.MARK_1_BLUEPRINT) == 1,
				"Mark 1 blueprint stamps with no prerequisite");

		// Mark 2 is locked until the entire Mark 1 suit is built.
		player.getInventory().add(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.BLANK_BLUEPRINT));
		com.herocraft.mod.ironman.TonyStark.stampBlueprint(player, "mark_2");
		helper.assertTrue(player.getInventory().countItem(com.herocraft.mod.ironman.item.IronManItems.MARK_2_BLUEPRINT) == 0,
				"Mark 2 blueprint must be locked until the full Mark 1 suit is built");

		for (net.minecraft.world.item.ArmorItem.Type t : new net.minecraft.world.item.ArmorItem.Type[] {
				net.minecraft.world.item.ArmorItem.Type.HELMET, net.minecraft.world.item.ArmorItem.Type.CHESTPLATE,
				net.minecraft.world.item.ArmorItem.Type.LEGGINGS, net.minecraft.world.item.ArmorItem.Type.BOOTS }) {
			helper.assertFalse(com.herocraft.mod.ironman.TonyStark.hasFabricatedFullSuit(player, "mark_1"),
					"Mark 1 is not complete before its 4th piece");
			com.herocraft.mod.ironman.TonyStark.recordSuitPiece(player, "mark_1", t);
		}
		helper.assertTrue(com.herocraft.mod.ironman.TonyStark.hasBuilt(player, "mark_1"),
				"a full set of pieces promotes Mark 1 into builtSuits");

		com.herocraft.mod.ironman.TonyStark.stampBlueprint(player, "mark_2");
		helper.assertTrue(player.getInventory().countItem(com.herocraft.mod.ironman.item.IronManItems.MARK_2_BLUEPRINT) == 1,
		...
		player.getInventory().add(new ItemStack(com.herocraft.mod.ironman.item.IronManItems.BLANK_BLUEPRINT));
		com.herocraft.mod.ironman.TonyStark.stampBlueprint(player, "mark_iii");
		helper.assertTrue(player.getInventory().countItem(com.herocraft.mod.ironman.item.IronManItems.MARK_III_BLUEPRINT) == 0,
	...
	public void ironManFaceplateOnlyOpensWhileArmored(GameTestHelper helper) {
		...
		player.setItemSlot(net.minecraft.world.entity.EquipmentSlot.HEAD,
				new ItemStack(com.herocraft.mod.ironman.item.IronManItems.armor("mark_iii",
						net.minecraft.world.item.ArmorItem.Type.HELMET)));
		com.herocraft.mod.ironman.IronManFaceplate.toggle(player);
		helper.assertTrue(com.herocraft.mod.ironman.IronManFaceplate.isOpen(player),
				"H must retract the helmet while an Iron Man helmet is worn");
		com.herocraft.mod.ironman.IronManFaceplate.toggle(player);
		helper.assertFalse(com.herocraft.mod.ironman.IronManFaceplate.isOpen(player), "H again closes it");
	...
	public void heroTierBlocksExperimentalGrant(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		com.herocraft.mod.ironman.TonyStark.grant(player);
		helper.assertTrue(com.herocraft.mod.hero.HeroTiers.hasHeroTier(player), "Tony Stark = hero tier");
	...
	public void commandGrantWipesEveryTier(GameTestHelper helper) {
		...
		com.herocraft.mod.hero.HeroTiers.wipeAll(player);
		helper.assertFalse(com.herocraft.mod.hero.HeroTiers.hasExperimental(player), "experimental wiped");
		helper.assertFalse(com.herocraft.mod.hero.HeroTiers.hasHeroTier(player), "hero-tier wiped");
	...
	public void spiderEvolutionConsumesAllMutations(GameTestHelper helper) {
		ServerPlayer player = survivalMockPlayer(helper);
		ExperimentalPowers.grant(player, power(com.herocraft.mod.spider.SpiderMan.SPIDER_ADHESION_KEY));
		ExperimentalPowers.grant(player, power("power_03_flight"));

		helper.assertTrue(com.herocraft.mod.spider.SpiderMan.evolveFromAdhesion(player), "evolution succeeds");
		helper.assertFalse(com.herocraft.mod.hero.HeroTiers.hasExperimental(player),
				"every mutation is gone after becoming Spider-Man");
		helper.assertTrue(com.herocraft.mod.spider.SpiderMan.hasPower(player), "and Spider-Man is granted");
		helper.succeed();
	}
