...
import com.herocraft.mod.firearm.FirearmHooks;
import com.herocraft.mod.firearm.FirearmReload;
import com.herocraft.mod.firearm.FirearmShooting;
import com.herocraft.mod.firearm.FirearmStack;
import com.herocraft.mod.firearm.Firearms;
import com.herocraft.mod.firearm.HeadshotResolver;
import com.herocraft.mod.firearm.item.FirearmItems;
import com.herocraft.mod.punisher.Punisher;
import com.herocraft.mod.punisher.PunisherConfig;
import com.herocraft.mod.punisher.ability.PunisherAdrenaline;
import com.herocraft.mod.punisher.ability.PunisherC4;
import com.herocraft.mod.punisher.ability.PunisherRoll;
import com.herocraft.mod.punisher.ability.PunisherSuppressive;
 ...
 * infinite-reserve seam, headshot resolution, and the power's ability gates + lifecycle cleanup.
...
public class PunisherGameTests implements FabricGameTest {
	...
	@GameTest(template = EMPTY_STRUCTURE)
	public void infiniteReserveHookOnlyForPunisher(GameTestHelper helper) {
		ServerPlayer punisher = punisher(helper);
		ServerPlayer plain = plain(helper);
		helper.assertTrue(FirearmHooks.get().infiniteReserve(punisher), "a Punisher has infinite reserve");
		helper.assertFalse(FirearmHooks.get().infiniteReserve(plain), "a normal player does not");
		helper.succeed();
	}
	...
	public void powerPersistsAndIsHeroTierExclusive(GameTestHelper helper) {
		ServerPlayer p = punisher(helper);
		helper.assertTrue(com.herocraft.mod.hero.HeroTiers.hasHeroTier(p), "the Punisher counts as a Hero-Tier power");
		com.herocraft.mod.hero.HeroTiers.wipeAll(p);
	...
	public void trainingCompletionGrantsThePower(GameTestHelper helper) {
		...
		com.herocraft.mod.punisher.VigilanteTraining.onHeadshot(p); // any progress call re-checks completion
