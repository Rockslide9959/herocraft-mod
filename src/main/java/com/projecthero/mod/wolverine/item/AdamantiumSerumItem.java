...
/**
 * The Adamantium Serum: the one way to become Wolverine. It does not create a healing factor, it
...
public class AdamantiumSerumItem extends Item {
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		if (Wolverine.hasPower(sp)) {
			sp.displayClientMessage(Component.translatable("message.projecthero.adamantium_serum.already")
					.withStyle(ChatFormatting.GRAY), true);
			return InteractionResultHolder.fail(held);
		}
		if (!Wolverine.hasSuperRegeneration(sp)) {
			sp.displayClientMessage(Component.translatable("message.projecthero.adamantium_serum.no_regeneration")
	...
	public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
		tooltip.add(Component.translatable("item.projecthero.adamantium_serum.desc1")
				.withStyle(ChatFormatting.DARK_GRAY));
		tooltip.add(Component.translatable("item.projecthero.adamantium_serum.desc2")
