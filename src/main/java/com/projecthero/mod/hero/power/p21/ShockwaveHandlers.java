		...
		AbilityHandlers.register(KEY, "repulsion_field", new AbilityHandler() {
			...
			public void onActivate(AbilityContext ctx) {
				...
				if (!ModeMeter.hasCharge(ctx, "repulsion_field", 30.0f)) {
					ctx.actionBar("message.projecthero.shockwave.repulsion_low");
					return;
				}
			...
			public void onServerTick(AbilityContext ctx) {
				...
				if (!ModeMeter.drain(ctx, "repulsion_field", MAX_REPULSION, REPULSION_DRAIN)) {
					ctx.setResource("repelling", 0, 1);
					ctx.actionBar("message.projecthero.shockwave.repulsion_out");
				}
			}
		});
		...
		com.projecthero.mod.hero.PowerPassives.register(KEY, (player, active) -> {
			if (active) {
				com.projecthero.mod.hero.power.PowerToggles.modifier(player,
						...
						com.projecthero.mod.ProjectHeroMod.id("shockwave_kb"), 0.2,
