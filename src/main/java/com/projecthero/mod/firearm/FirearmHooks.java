 ...
 * neutral (a plain player with a crafted gun). {@code com.projecthero.mod.punisher} installs a real
...
public interface FirearmHooks {
	...
	/**
	 * True if this player draws reloads from a personal, engine-external reserve pool rather than from
	 * ammo items carried in the inventory (the Punisher's per-gun reserve -- see
	 * {@code PunisherAmmoReserve}). When true, {@link #personalReserveCount} and
	 * {@link #personalReserveTake} are authoritative for that player.
	 */
	default boolean usesPersonalReserve(ServerPlayer player) {
		return false;
	}

	/** Rounds of {@code kind} in this player's personal reserve right now (only meaningful when
	 *  {@link #usesPersonalReserve} is true). */
	default int personalReserveCount(ServerPlayer player, AmmoKind kind) {
		return 0;
	}

	/** Take up to {@code want} rounds of {@code kind} from this player's personal reserve; returns the
	 *  amount actually removed. */
	default int personalReserveTake(ServerPlayer player, AmmoKind kind, int want) {
		return 0;
	}
