...
public class GravekeeperShieldItem extends ShieldItem {
	...
	public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
		tooltip.add(Component.translatable("item.projecthero.gravekeeper_shield.undead",
				Math.round(UNDEAD_REDUCTION * 100)).withStyle(ChatFormatting.GREEN));
		tooltip.add(Component.translatable("item.projecthero.gravekeeper_shield.acid",
				Math.round(ACID_REDUCTION * 100)).withStyle(ChatFormatting.GREEN));
		tooltip.add(Component.translatable("item.projecthero.gravekeeper_shield.charge")
