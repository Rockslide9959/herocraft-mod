 ...
 * see {@link com.herocraft.mod.network.ModNetworking} for the server-side re-validation and stamping.
 */
public class BlankBlueprintItem extends Item {
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		if (!TonyStark.hasPower(serverPlayer)) {
			serverPlayer.displayClientMessage(Component
					.translatable("message.herocraft.ironman.craft_requires_tony_stark").withStyle(ChatFormatting.RED), true);
	...
	public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
		tooltip.add(Component.translatable("item.herocraft.blank_blueprint.hint")
