...
import static com.herocraft.mod.ironman.fabricator.FabricationRecipe.in;
import static com.herocraft.mod.ironman.fabricator.FabricationRecipe.of;
...
public final class FabricatorRecipes {
	...
	public static FabricationRecipe find(Container container, int operatorTechLevel, int selectedPiece) {
		...
		int blueprintTier = blueprint.getItem() instanceof com.herocraft.mod.ironman.item.BlueprintItem bp
				? bp.techLevel() : 0;
		...
		for (FabricationRecipe recipe : all()) {
			...
			if (recipe.result().getItem() instanceof com.herocraft.mod.ironman.item.IronManArmorItem) {
				continue;
			}
	...
	public static FabricationRecipe selectedArmorRecipe(Container container, int selectedPiece) {
		...
		if (!(container.getItem(10).getItem() instanceof com.herocraft.mod.ironman.item.BlueprintItem bp)
				|| bp.suitId() == null) {
			return null;
		}
