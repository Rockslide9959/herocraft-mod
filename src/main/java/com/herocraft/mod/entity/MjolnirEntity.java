...
import com.herocraft.mod.hammer.MjolnirStatus;
import com.herocraft.mod.item.ModItems;
import com.herocraft.mod.power.ThorFeedback;
...
public class MjolnirEntity extends ThrowableItemProjectile {
	...
	private static final String TAG_STATE = "HeroCraftState";
	private static final String TAG_TICKS_ALIVE = "HeroCraftTicksAlive";
	private static final String TAG_PICKUP_DELAY = "HeroCraftPickupDelay";
	private static final String TAG_PICKUP_ARMED = "HeroCraftPickupArmed";
	private static final String TAG_IMPACT_TICKS = "HeroCraftImpactTicks";

	/**
	 * What the hammer is currently doing -- the single switch the rest of the class reads. Mirrored
	 * into the persistent {@link com.herocraft.mod.hammer.HammerRecord} as a {@link MjolnirStatus} so
	 ...
	 *                         if they walked in and stayed. {@link com.herocraft.mod.worldgen.CraterAmbience}
	 ...
	 * {@link com.herocraft.mod.hammer.MjolnirRecall}.
	 */
	...
	/**
	 * Called by the "call hammer" keybind (via {@link com.herocraft.mod.hammer.MjolnirRecall}) to
	...
	public void recall(Player summoner) {
		...
		if (level() instanceof ServerLevel serverLevel) {
			// A rising whoosh with electric energy -- see HeroCraftSounds' javadoc for the placeholder
			...
			serverLevel.playSound(null, this.getX(), this.getY(), this.getZ(),
					HeroCraftSounds.MJOLNIR_RECALL, SoundSource.PLAYERS, 0.8f, 1.0f);
			serverLevel.playSound(null, this.getX(), this.getY(), this.getZ(),
	 ...
	 * (see {@link com.herocraft.mod.power.ThorPassives}'s class javadoc). Unworthy players get the
	...
	private void catchBy(Player player) {
		...
		level().playSound(null, player.blockPosition(), SoundEvents.ITEM_PICKUP, SoundSource.PLAYERS, 1.0f, 1.2f);
		level().playSound(null, player.blockPosition(), HeroCraftSounds.MJOLNIR_CATCH, SoundSource.PLAYERS, 0.8f, 1.2f);
