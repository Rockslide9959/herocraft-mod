...
/**
 * Reserve-ammunition bookkeeping. A Punisher ({@link FirearmHooks#infiniteReserve}) never spends an
 * ammo item; every other player must carry the matching {@link AmmoKind} item and a reload consumes
 * only as many rounds as it takes to top the magazine off -- partial reloads are allowed (spec
 * section 35).
 */
public final class FirearmAmmo {
	...
	public static int reserveCount(ServerPlayer player, AmmoKind kind) {
		if (FirearmHooks.get().infiniteReserve(player)) {
