...
public class ArachnidMutagenItem extends Item {
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		if (SpiderMan.hasPower(sp)) {
			sp.displayClientMessage(Component.translatable("message.herocraft.arachnid_mutagen.already")
					.withStyle(ChatFormatting.GRAY), true);
			return InteractionResultHolder.fail(held);
		}
		if (!SpiderMan.hasSpiderAdhesion(sp)) {
			sp.displayClientMessage(Component.translatable("message.herocraft.arachnid_mutagen.no_adhesion")
	...
	public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
		tooltip.add(Component.translatable("item.herocraft.arachnid_mutagen.desc1")
				.withStyle(ChatFormatting.DARK_GRAY));
		tooltip.add(Component.translatable("item.herocraft.arachnid_mutagen.desc2")
