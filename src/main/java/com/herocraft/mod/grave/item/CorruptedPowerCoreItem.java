...
public class CorruptedPowerCoreItem extends Item {
	...
	public Component getName(ItemStack stack) {
		...
		return Component.translatable("item.herocraft.corrupted_power_core.named", BossPowers.displayName(key));
	}
	...
	public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
		...
		String key = powerKey(stack);
		if (!key.isEmpty()) {
			tooltip.add(Component.translatable("item.herocraft.corrupted_power_core.power",
					BossPowers.displayName(key)).withStyle(ChatFormatting.LIGHT_PURPLE));
		}
		tooltip.add(Component.translatable("item.herocraft.corrupted_power_core.hint")
