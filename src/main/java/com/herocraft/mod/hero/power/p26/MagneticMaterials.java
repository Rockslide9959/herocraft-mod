 ...
 * anchor) and netherite (reactive but very resistant). {@link com.herocraft.mod.item.ModItems#MJOLNIR
 * Mjolnir} is explicitly excluded from every check so Magnetism can never touch Thor's hammer.
 *
 * <p>Data-driven where it can be: the {@code #herocraft:magnetic} block and item tags hold the
 * concrete vanilla list; this class adds the tier/entity/special-case logic on top.
 */
public final class MagneticMaterials {
	public static final TagKey<Block> MAGNETIC_BLOCKS = TagKey.create(Registries.BLOCK, HeroCraftMod.id("magnetic"));
	public static final TagKey<Item> MAGNETIC_ITEMS = TagKey.create(Registries.ITEM, HeroCraftMod.id("magnetic"));
