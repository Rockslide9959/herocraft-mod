 ...
 * {@link com.herocraft.mod.ironman.item.IronManArmorItem} (the five Iron Man marks) and
 * {@link com.herocraft.mod.item.ThorArmorItem} -- each set stays its own item with its own id,
...
public abstract class SuperheroArmorItem extends ArmorItem implements GeoItem {
	...
	/**
	 * Installed by the client ({@code HeroCraftModClient}) so common code can hand GeckoLib a
