...
public final class HeroCommand {
	...
	private static Power resolve(CommandContext<CommandSourceStack> c) {
		...
		return power != null ? power : Powers.get(com.herocraft.mod.HeroCraftMod.id(key));
	}
	...
	private static int grant(CommandContext<CommandSourceStack> c, ServerPlayer target) {
		...
		boolean hadAny = com.herocraft.mod.hero.HeroTiers.hasExperimental(target)
				|| com.herocraft.mod.hero.HeroTiers.hasHeroTier(target);
		...
		com.herocraft.mod.hero.PowerPassives.reconcileActive(target);
	...
	private static int revoke(CommandContext<CommandSourceStack> c, ServerPlayer target) {
		...
		if (rawKey.equalsIgnoreCase("all")) {
			...
			if (com.herocraft.mod.hero.power.HeroFlight.isFlying(target)) {
				com.herocraft.mod.hero.power.HeroFlight.setFlying(target, false);
			}
		...
		if (removed) {
			...
			target.setAttached(com.herocraft.mod.attachment.ModAttachments.EXPERIMENTAL_STATE, state);
			com.herocraft.mod.hero.PowerPassives.reconcileActive(target);
		}
	...
	private static int grantHeroByKey(CommandContext<CommandSourceStack> c, ServerPlayer target, String hero) {
		...
		switch (hero) {
			...
			case "punisher" -> {
				boolean ok = com.herocraft.mod.punisher.Punisher.grant(target);
	...
	private static int revokeHeroByKey(CommandContext<CommandSourceStack> c, ServerPlayer target, String hero) {
		...
		switch (hero) {
			...
			case "punisher" -> com.herocraft.mod.punisher.Punisher.revoke(target);
			default -> {
				c.getSource().sendFailure(Component.literal("Unknown Hero-Tier power (thor, iron_man, spider_man, max_steel)"));
				return 0;
			}
	...
	private static int serum(CommandContext<CommandSourceStack> c, ServerPlayer target) {
		...
		net.minecraft.world.item.ItemStack stack = net.minecraft.world.item.alchemy.PotionContents.createItemStack(
				net.minecraft.world.item.Items.POTION, com.herocraft.mod.hero.mutation.ModSerums.serum(power));
	...
	private static int research(CommandContext<CommandSourceStack> c, ServerPlayer target) {
		...
		com.herocraft.mod.hero.mutation.MutationManager.studyResearchNote(target, power);
		return 1;
	}

	private static int status(CommandContext<CommandSourceStack> c, ServerPlayer target) {
		c.getSource().sendSuccess(() -> Component.literal(com.herocraft.mod.hero.mutation.MutationManager.describe(target)), false);
		return 1;
	}
}
