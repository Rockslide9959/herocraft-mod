...
public final class SuperheroCommand {
	...
	private static int worthiness(CommandContext<CommandSourceStack> c) throws CommandSyntaxException {
		...
		c.getSource().sendSuccess(() -> Component.translatable("commands.herocraft.superhero.worthiness",
				score, Worthiness.THRESHOLD, Component.translatable(worthy
						? "commands.herocraft.superhero.worthy" : "commands.herocraft.superhero.unworthy")
	...
	private static int removeReactor(CommandContext<CommandSourceStack> c) throws CommandSyntaxException {
		...
		if (!TonyStark.hasPower(p)) {
			c.getSource().sendFailure(Component.translatable("commands.herocraft.superhero.no_reactor"));
			return 0;
		}
		if (com.herocraft.mod.ironman.IronManArmor.wearingAnyIronMan(p)) {
			c.getSource().sendFailure(Component.translatable("commands.herocraft.superhero.remove_suit_first"));
			return 0;
		}
		TonyStark.revoke(p);
		...
		c.getSource().sendSuccess(() -> Component.translatable("commands.herocraft.superhero.reactor_removed")
