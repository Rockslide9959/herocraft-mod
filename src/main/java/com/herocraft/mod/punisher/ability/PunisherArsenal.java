...
import com.herocraft.mod.network.PunisherArsenalOpenPayload;
...
public final class PunisherArsenal {
	...
	public static void equip(ServerPlayer player, String weaponId) {
		...
		if (!Punisher.weaponUnlocked(player, weaponId)) {
			player.displayClientMessage(Component.translatable("message.herocraft.punisher.weapon_locked")
	...
	private static void announce(ServerPlayer player, String weaponId) {
		...
		player.displayClientMessage(Component.translatable("item.herocraft." + weaponId)
				.withStyle(ChatFormatting.GRAY), true);
	}
