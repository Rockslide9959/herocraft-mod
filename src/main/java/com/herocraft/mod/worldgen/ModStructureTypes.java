 ...
 * themselves live in {@code data/herocraft/worldgen/structure(_set)/mjolnir_crater.json} (dynamic
 * registries, loaded from datapack JSON), but the {@link StructureType} that JSON references by
 * {@code "type": "herocraft:mjolnir_crater"} has to be a real Java object registered into
...
public final class ModStructureTypes {
	...
	public static final StructureType<com.herocraft.mod.maxsteel.worldgen.SteelCrashSiteStructure> STEEL_CRASH_SITE =
			register("steel_crash_site", com.herocraft.mod.maxsteel.worldgen.SteelCrashSiteStructure.CODEC);

	/** Punisher: the rare buried bunker holding the Vigilante Training Manual. */
	public static final StructureType<com.herocraft.mod.punisher.worldgen.VigilanteSafehouseStructure> VIGILANTE_SAFEHOUSE =
			register("vigilante_safehouse", com.herocraft.mod.punisher.worldgen.VigilanteSafehouseStructure.CODEC);

	/** Symbiote: the rare impact crater a Symbiote rode down in. */
	public static final StructureType<com.herocraft.mod.symbiote.worldgen.SymbioteMeteorStructure> SYMBIOTE_METEOR =
			register("symbiote_meteor", com.herocraft.mod.symbiote.worldgen.SymbioteMeteorStructure.CODEC);

	/** Symbiote: the half-buried containment lab with a Symbiote in its cell. */
	public static final StructureType<com.herocraft.mod.symbiote.worldgen.SymbioteLabStructure> SYMBIOTE_LAB =
			register("symbiote_lab", com.herocraft.mod.symbiote.worldgen.SymbioteLabStructure.CODEC);
	...
	public static void initialize() {
		...
		// exists so HeroCraftMod has an explicit, readable init call.
	}

	private static <S extends Structure> StructureType<S> register(String path, MapCodec<S> codec) {
		ResourceKey<StructureType<?>> key = ResourceKey.create(Registries.STRUCTURE_TYPE, HeroCraftMod.id(path));
