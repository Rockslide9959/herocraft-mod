...
import com.herocraft.mod.client.ModKeyBindings;
	...
	public static void render(GuiGraphics g, DeltaTracker delta) {
		...
		if (!(helmet.getItem() instanceof IronManArmorItem helmetPiece)) {
			...
			if (!wearingAnyIronMan(player)) {
				g.drawString(client.font, Component.translatable("hud.herocraft.ironman.call_armor",
		...
		float maxIntegrity = com.herocraft.mod.ironman.IronManEnergy.maxIntegrity(suitId);
		...
		if (energyFrac < 0.35f || integrityFrac < 0.35f) {
			if ((now % 20) < 13) {
				g.drawString(client.font, Component.translatable("hud.herocraft.ironman.repair_warning")
		...
		float maxHeat = com.herocraft.mod.ironman.ability.IronManAbilities.flamethrowerMaxHeat(suit);
		if ((hasAbility(suit, com.herocraft.mod.ironman.ability.IronManAbilities.FLAMETHROWER) || suit.hasWeaponWheel())
		...
		if (state.timedFlightUntil > now) {
			float total = com.herocraft.mod.ironman.ability.IronManAbilities.TIMED_FLIGHT_TICKS;
		...
		if (state.overloadUntil > now) {
			float total = com.herocraft.mod.ironman.ability.IronManAbilities.OVERLOAD_TICKS;
			float overFrac = clamp((state.overloadUntil - now) / total);
			g.drawString(client.font, Component.translatable("hud.herocraft.ironman.overloaded")
			...
			float total = com.herocraft.mod.ironman.ability.IronManAbilities.WRIST_LASER_TICKS;
			float laserFrac = clamp((state.wristLaserUntil - now) / total);
			g.drawString(client.font, Component.translatable("hud.herocraft.ironman.ability.wrist_laser")
		...
		if (suit.altitudeCeiling() > 0.0) {
			...
			if (player.getY() >= ceiling) {
				g.drawString(client.font, Component.translatable("hud.herocraft.ironman.systems_frozen")
						.withStyle(ChatFormatting.RED, ChatFormatting.BOLD), X, y, 0xFFFF5555);
				y += 12;
			} else if (player.getY() >= ceiling - 20.0) {
				g.drawString(client.font, Component.translatable("hud.herocraft.ironman.altitude_warning",
		...
		for (int i = 0; i < 6; i++) {
			...
			String effectiveId = com.herocraft.mod.ironman.ability.IronManAbilities.WEAPON_WHEEL_SLOT.equals(abilityId)
			...
			String nameKey = "hud.herocraft.ironman.ability." + effectiveId;
			if (com.herocraft.mod.ironman.ability.IronManAbilities.REPULSOR_BLAST.equals(abilityId)
					&& suit.repulsorWindupTicks() > 0) {
				nameKey = "hud.herocraft.ironman.ability.repulsor_blast_windup"; // Mark 2: no charged variant
			}
			if (com.herocraft.mod.ironman.ability.IronManAbilities.MOB_HIGHLIGHT_TOGGLE.equals(abilityId)
					&& suit.hasWristLaser()) {
				nameKey = "hud.herocraft.ironman.ability.mob_highlight_toggle.wristlaser"; // Mark 4: sneak + V
			}
			...
			if (com.herocraft.mod.ironman.ability.IronManAbilities.WEAPON_WHEEL_SLOT.equals(abilityId)) {
				line = line.copy().append(Component.translatable("hud.herocraft.ironman.wheel_marker")
		...
		if (suit.passiveHighlightRange() > 0.0) {
			g.drawString(client.font, Component.translatable("hud.herocraft.ironman.passive_highlight",
					(int) suit.passiveHighlightRange()).withStyle(ChatFormatting.AQUA), X, y, 0xFF7FE9FF);
			y += 10;
		}
		if (suit.hasWeaponWheel()) {
			g.drawString(client.font, Component.translatable("hud.herocraft.ironman.wheel_bound",
					Component.translatable("hud.herocraft.ironman.ability." + state.weaponWheelChoice))
		...
		if (player.getAttachedOrElse(ModAttachments.IRON_MAN_BLADES, false)) {
			g.drawString(client.font, Component.translatable("hud.herocraft.ironman.blades_active")
					.withStyle(ChatFormatting.AQUA), X, y, 0xFF7FE9FF);
			y += 10;
		}
		if (player.getAttachedOrElse(ModAttachments.IRON_MAN_FACEPLATE_OPEN, false)) {
			g.drawString(client.font, Component.translatable("hud.herocraft.ironman.faceplate_open")
					.withStyle(ChatFormatting.GRAY), X, y, 0xFFB8C0E0);
			y += 10;
		}

		// "changes 13": mob-highlight toggle state + a Micro-Missiles ammo / reload readout.
		if (hasAbility(suit, com.herocraft.mod.ironman.ability.IronManAbilities.MOB_HIGHLIGHT_TOGGLE)) {
			boolean on = state.mobHighlightOn;
			g.drawString(client.font, Component.translatable(on
					? "hud.herocraft.ironman.highlight_on" : "hud.herocraft.ironman.highlight_off")
		...
		if (suit.missileCount() > 0) {
			...
			Component missiles = mCd > 0
					? Component.translatable("hud.herocraft.ironman.missiles_reload",
							String.format(java.util.Locale.ROOT, "%.1f", mCd / 20.0))
							.withStyle(ChatFormatting.RED)
					: Component.translatable("hud.herocraft.ironman.missiles", suit.missileCount())
		...
		if (energyFrac <= 0.001f || integrity <= 0.001f || state.overloadUntil > now) {
			g.drawString(client.font, Component.translatable("hud.herocraft.ironman.offline")
