...
public class SuperheroArmorRenderer extends GeoArmorRenderer<SuperheroArmorItem> {
	...
	protected void applyBoneVisibilityBySlot(EquipmentSlot slot) {
			...
			case HEAD -> {
				...
				boolean ironManFaceplate = getCurrentEntity() instanceof Player p
						&& p.getItemBySlot(EquipmentSlot.HEAD).getItem()
								instanceof com.herocraft.mod.ironman.item.IronManArmorItem
						&& com.herocraft.mod.ironman.IronManFaceplate.isOpen(p);
				boolean maxSteelHelmet = getCurrentEntity() instanceof Player mp
						...
						&& com.herocraft.mod.maxsteel.MaxSteelFaceplate.isOpen(mp);
				...
				boolean spiderMask = getCurrentEntity() instanceof Player sp
						...
						&& com.herocraft.mod.spider.SpiderMask.isOpen(sp);
		...
		if (getCurrentEntity() instanceof Player player
				&& player.getItemBySlot(slot).getItem() instanceof com.herocraft.mod.maxsteel.item.MaxSteelArmorItem
				&& com.herocraft.mod.client.maxsteel.MaxSteelReveal.isRevealing(player)) {
			for (String bone : com.herocraft.mod.client.maxsteel.MaxSteelReveal.boneNames()) {
				if (com.herocraft.mod.client.maxsteel.MaxSteelReveal.hidden(player, bone)) {
		...
		if (getCurrentEntity() instanceof Player symbiotePlayer
				&& symbiotePlayer.getItemBySlot(slot).getItem() instanceof com.herocraft.mod.spider.item.SymbioteArmorItem
				&& com.herocraft.mod.client.symbiote.SymbioteReveal.isRevealing(symbiotePlayer)) {
			for (String bone : com.herocraft.mod.client.symbiote.SymbioteReveal.boneNames()) {
				if (com.herocraft.mod.client.symbiote.SymbioteReveal.hidden(symbiotePlayer, bone)) {
					getGeoModel().getBone(bone).ifPresent(b -> b.setHidden(true));
				}
			}
		}
	...
	/**
	 * Stash the wearer for {@link com.herocraft.mod.armor.ArmorRenderContext} before GeckoLib resolves
	...
	public void prepForRender(net.minecraft.world.entity.Entity entity, net.minecraft.world.item.ItemStack stack,
		...
		com.herocraft.mod.armor.ArmorRenderContext.set(entity);
	...
	public void renderToBuffer(PoseStack poseStack, @Nullable VertexConsumer buffer, int packedLight,
		...
		try {
			...
			if (getCurrentEntity() instanceof Player p
					&& com.herocraft.mod.client.maxsteel.MaxSteelReveal.isStealthed(p)) {
				return;
			}
			super.renderToBuffer(poseStack, buffer, packedLight, packedOverlay, colour);
		} finally {
			com.herocraft.mod.armor.ArmorRenderContext.clear();
		}
	}

	/**
	 * Shows or hides the entire head armour -- the {@code helmet} shell and the {@code faceplate}
	 * visor together -- for the open-faceplate state ({@link com.herocraft.mod.ironman.IronManFaceplate}).
	 *
