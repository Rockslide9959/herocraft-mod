...
/**
 * The firearm HUD: weapon name, magazine / capacity, reserve (an inventory count, or the Punisher's
 * personal per-gun reserve pool shown as {@code have / capacity}), a
...
public final class FirearmHud {
	...
	public static void render(GuiGraphics g, DeltaTracker delta) {
		...
		Component name = Component.translatable("item.projecthero." + data.id);
		...
		var punisherState = mc.player.getAttachedOrElse(
				com.projecthero.mod.attachment.ModAttachments.PUNISHER_STATE, null);
		boolean punisher = punisherState != null && punisherState.hasPower;
		String reserve;
		if (punisher) {
			// v0.9.22: personal per-gun reserve (3 magazines, regenerating) instead of the old ∞.
			int have = com.projecthero.mod.punisher.PunisherAmmoReserve.count(punisherState, data.ammo);
			int cap = com.projecthero.mod.punisher.PunisherAmmoReserve.capacity(data.ammo);
			reserve = have + " / " + cap + " reserve";
		} else {
			reserve = reserveCount(mc, data.ammo) + " reserve";
		}
		...
		if (reloading) {
			...
			Component rl = Component.translatable("firearm.projecthero.hud.reloading").withStyle(ChatFormatting.GRAY);
		...
		if (!reloading && lastFired != null && data.cycleTicks > 0 && now - lastFired < data.cycleTicks) {
			Component cy = Component.translatable("firearm.projecthero.hud.cycling").withStyle(ChatFormatting.GRAY);
			g.drawString(mc.font, cy, x - mc.font.width(cy), y + 42, 0xFF999999, true);
		}

		if (headshotUntil > now) {
			Component hs = Component.translatable("firearm.projecthero.hud.headshot").withStyle(ChatFormatting.RED, ChatFormatting.BOLD);
