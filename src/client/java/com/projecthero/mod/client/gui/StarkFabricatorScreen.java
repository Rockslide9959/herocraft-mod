...
public class StarkFabricatorScreen extends AbstractContainerScreen<StarkFabricatorMenu> {
	...
	private void renderPieceButtons(GuiGraphics g, int x, int y, int mouseX, int mouseY) {
		...
		for (int i = 0; i < 4; i++) {
			...
			Component label = Component.translatable("screen.projecthero.stark_fabricator.piece." + PIECE_KEYS[i]);
		...
		g.drawString(this.font, Component.translatable("screen.projecthero.stark_fabricator.view_more"),
	...
	protected void renderLabels(GuiGraphics g, int mouseX, int mouseY) {
		...
		g.drawString(this.font, Component.translatable("screen.projecthero.stark_fabricator.blueprint"), 8, 27, LABEL, false);
		g.drawString(this.font, Component.translatable("screen.projecthero.stark_fabricator.output"), 108, 27, LABEL, false);
		g.drawString(this.font, Component.translatable("screen.projecthero.stark_fabricator.energy_label"),
		...
		if (menu.maxProgress() > 0) {
			...
			status = Component.translatable("screen.projecthero.stark_fabricator.fabricating", pct)
					.withStyle(ChatFormatting.GREEN);
			statusColor = 0xFFB8FFD0;
		} else if (hasSuitBlueprint() && menu.selectedPiece() == 0) {
			status = Component.translatable("screen.projecthero.stark_fabricator.pick_piece").withStyle(ChatFormatting.GRAY);
			statusColor = 0xFF8A93A8;
		} else {
			status = Component.translatable("screen.projecthero.stark_fabricator.idle_hint").withStyle(ChatFormatting.GRAY);
			statusColor = 0xFF8A93A8;
		}
		int sy = 84;
	...
	private List<Component> buildInfoLines(IronManSuit suit) {
		...
		if (recipe != null) {
			int piece = menu.selectedPiece();
			out.add(Component.translatable("screen.projecthero.stark_fabricator.needs_for",
					Component.translatable("screen.projecthero.stark_fabricator.piece." + PIECE_KEYS[piece - 1]))
			...
			out.add(Component.translatable("screen.projecthero.stark_fabricator.cost", recipe.energyCost(),
					String.format(Locale.ROOT, "%.1f", recipe.timeTicks() / 20.0)).withStyle(ChatFormatting.DARK_GRAY));
			out.add(Component.empty());
		} else {
			out.add(Component.translatable("screen.projecthero.stark_fabricator.pick_piece").withStyle(ChatFormatting.GRAY));
			out.add(Component.empty());
		}
		if (suit != null) {
			out.add(Component.translatable("screen.projecthero.stark_fabricator.suit_stats").withStyle(ChatFormatting.GOLD));
	...
	private Component statLine(String key, String value) {
		return Component.literal(" ")
				.append(Component.translatable("screen.projecthero.stark_fabricator.stat." + key))
