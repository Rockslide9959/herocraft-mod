...
**Two Primary slots.** A player holds up to `HeroTiers.PRIMARY_SLOTS` (2) Primary powers. Each Hero-Tier power is one
slot; the experimental mutations are one group in one slot (still capped by `mutationCapacity`, default 3).

- `HeroTiers.claimPrimary(player, key)` (called by every hero's grant routine): strips all mutations, then keeps
  at most one *other* hero (the oldest is revoked), records the new hero last in the
  `ModAttachments.PRIMARY_ORDER` attachment, and removes the Symbiote unless the new power is Spider-Man.
- `HeroTiers.claimExperimental(player)` (mutation serum): keeps at most one hero (the newest) beside the mutation
  group and removes the Symbiote; returns whether anything was replaced.
- `HeroTiers.wipeAll` still strips everything (Power Suppressor, `/heropower grant` for experimental powers).
  The Power Suppressor now also calls `Symbiote.remove`.
- Commands: `/thor`, `/ironman`, `/maxsteel`, `/punisher`, `/greenlantern`, `/heropower grant <hero>` claim a slot
  like natural acquisition (no more wipe-everything-first).

### Symbiote (v0.11.15)

- **Sounds:** every effect goes through `SymbioteSounds` (sculk block spread/charge + warden tendril clicks +
  warden impact) instead of slime/swim/fishing-line "water" sounds.
- **Speech** (`SymbioteDialogue.speak`) is shown on the action bar; `SymbioteDialogue.say(player, id)` fires an event
  line immediately.
- **Regeneration II** only while hurt (and `SymbioteVitalsManager.usable`); each 1 s pulse costs 2 Biomass
  (`SymbiotePassives.tick`).
- **Auto-equip** (`Symbiote.autoEquip` / `onIncomingHit`): a hit of >3, one leaving <5 hearts, a fatal one, or health
  ...
  (v0.11.18: auto-wraps no longer retract by themselves.) (v0.12.15: Resistance I removed; the suit instead cuts all damage taken by 10% in `SymbioteDamageRules`.)
...
- **Resurrection** (`Symbiote.tryResurrect`, from `ALLOW_DEATH` after Protocol Phoenix / Max Steel revive): costs
  ...
  within 20 blocks outward and draws tendril particle rays.
- **Sound attacks** (`SonicVulnerability`, now 100 ticks): `Symbiote.tickSonic` hard-deactivates the suit, locks the
...
- **Symbiote Vial** (`symbiote/item/SymbioteVialItem`): empty + sneak-use extracts the bond, empty + right-click on a
  free `SymbioteEntity` captures it, filled + use bonds (`SymbioteBonding.attemptFromVial`). Recipe: glass around two
  iron blocks.

### Thor (v0.11.15)

- Mjolnir Crater structure set: spacing 100 / separation 40 / frequency 0.5 (was 48 / 16) -- roughly nine times rarer.
- `hammer/MjolnirSeer`: Sneak + use a villager (any `AbstractVillager`) with a Lightning Rod opens a one-offer
  `Merchant` (25 emeralds -> a "Directions to Mjolnir" paper); `notifyTrade` tells the player the compass point of
  the nearest free hammer (`MjolnirRegistry.freeHammers`) or unvisited crater (scans the structure set's own placement
  grid; skips craters in `CraterSpawnState`).
- A Hero of the Village lifting a hammer bound to another player (`Worthiness.wouldAscend(player, stack)` is false)
  carries it without ascending, binding or consuming the effect; `WorthinessEnforcer` leaves it in their inventory
  while the effect lasts. Owner state is untouched.
- `ThorFeedback.bound`: "You are worthy." (gold, bold) then one line, "Mjolnir accepts you, the power of Thor is now
  yours." (blue, bold).
